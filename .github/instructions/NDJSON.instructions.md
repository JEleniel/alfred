---
description: 'Agent directives for json (newline-delimited JSON) formatting and handling.'
applyTo: '**/*.json'
---

# json Formatting & Handling

json (newline-delimited JSON) stores one complete JSON value per line (typically one object per line). Treat json as a streaming format: do not rewrite it into arrays or pretty-printed multi-line JSON.
When a consuming platform, tool, or specification requires a conflicting json convention, follow the consumer's requirements.
